...
?>
<META HTTP-EQUIV="Refresh" CONTENT="30; URL=users_popupH.php3?<?php echo($Refresh); ?>">
...
<CENTER>
	<A HREF="<?php echo("$From?Ver=H&L=$L"); ?>" TARGET="_blank"><?php echo(L_CHAT); ?></A>
	<P>
		...
		&nbsp;
		<A HREF="users_popupH.php3?<?php echo($ChangeBeeps_Reload); ?>"><IMG SRC="images/<?php if ($B == "0") echo("no"); ?>sound.gif" WIDTH=13 HEIGHT=13 ALIGN="MIDDLE" BORDER=0 ALT="<?php echo(L_BEEP); ?>"></A>
	</P>
</CENTER>
			...
			while(list($Other) = $DbLink->next_record())
			{
				if($Users->query("SELECT username, latin1, status FROM ".C_USR_TBL." WHERE room = '".addslashes($Other)."' ORDER BY username"))
				{
					if($Users->num_rows() > 0)
					{
						...
						echo("<A HREF=\"#\" onClick=\"expandIt('${id}'); return false\">");
