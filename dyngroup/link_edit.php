<?php
	...
	if ($action == 'Link Add') {
		$sql = "INSERT INTO dyn_links (cat1, attr1, cat2, attr2) VALUES ('$cat1', '$attr1', '$cat2', '$attr2')";
		$res = $db->query($sql);
		$action = '';
	}

	if (($action == 'Link Update') && ($editing == 'no')) {
		$sql = "UPDATE dyn_links SET cat1='$cat1', attr1='$attr1', cat2='$cat2', attr2='$attr2' WHERE id=$link_id";
		$res = $db->query($sql);
		$action = '';
		$editing = 'no';
	}
	...
	if ($action == 'Link Edit') {
		...
		if ($editing <> 'yes') {
			$sql = "SELECT * FROM dyn_links WHERE id=$link_id";
			$res = $db->query($sql);
	...
	if ($action == 'Link Delete') {
		$sql = "DELETE FROM dyn_links WHERE id=$link_id";
		$res = $db->query($sql);
		$action = '';
	}
...
<br>
<center><A href="admin.php">Return to Dynamic Group Home</a></center>
</BODY>
</HTML>
