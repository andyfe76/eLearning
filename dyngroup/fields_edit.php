<?php
	...
	if (($action == 'edit') && ($editing=='yes')) {
		$sql = "SELECT * FROM dyn_definitions WHERE id=$id";
		$res = $db->query($sql);
	...
	if ($action == 'Add') {
		$sql = "INSERT INTO dyn_definitions (cat, attr, description, tbl, field) VALUES ('$cat', '$attr', '$desc', '$table', '$field')";
		$res = $db->query($sql);
		$action = '';
	}
	
	if ($action == 'Update') {
		$sql = "UPDATE dyn_definitions SET cat='$cat', attr='$attr', description='$desc', tbl='$table', field='$field' WHERE id=$id";
		$res = $db->query($sql);
		$action = 'Add';
	}

 	if ($action == 'delete') {
 		$sql = "DELETE FROM dyn_definitions WHERE id=$id";
 		$action = 'Add';
 	}
?>
...
<?php
	$sql = "SELECT * FROM dyn_definitions";
	$res = $db->query($sql);
	while ($row =$res->fetchRow(DB_FETCHMODE_ASSOC)) {
	?>
...
<br>
<center><A href="admin.php">Return to Dynamic Group</a></center>
</BODY>
</HTML>
