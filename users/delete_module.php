...
if (!$_GET['d']) {
	$sql = "SELECT name from course_groups WHERE group_id=$_GET[mod]";
	$res = $db->query($sql);
	$row = $res->fetchRow();
	$mod_name = $row[0];
	$warnings[]= array(AT_WARNING_SURE_DELETE_MODULE1, $mod_name);
	print_warnings($warnings);
	$mod = $_GET['mod'];
	//phpinfo();
	if(ereg("admin" , $_SERVER[HTTP_REFERER])){
		if($_GET['member_id']){
			echo '<center><a href="'.$PHP_SELF.'?mod='.$mod.SEP.'d=1'.SEP.'ad=1'.SEP.'member_id='.$_GET['member_id'].'">'.$_template['yes_delete'].'</a> | <a href="users/admin/courses.php?f='.urlencode_feedback(AT_FEEDBACK_CANCELLED).SEP.'member_id='.$_GET['member_id'].'">'.$_template['no_cancel'].'</a></center>';
		}else{
			echo '<center><a href="'.$PHP_SELF.'?mod='.$mod.SEP.'d=1'.SEP.'ad=1">'.$_template['yes_delete'].'</a> | <a href="users/admin/courses.php?f='.urlencode_feedback(AT_FEEDBACK_CANCELLED).'">'.$_template['no_cancel'].'</a></center>';
		}
	}else{
		echo	'<center><a href="'.$PHP_SELF.'?mod='.$mod.SEP.'d=1'.'">'.$_template['yes_delete'].'</a> | <a href="users/?f='.urlencode_feedback(AT_FEEDBACK_CANCELLED).'">'.$_template['no_cancel'].'</a></center>';
	}
?>
	<!--center><a href="<?php echo $PHP_SELF.'?mod='.$mod.SEP.'d=1'; ?>"><?php echo $_template['yes_delete']; ?></a> | <a href="users/?f=<?php echo urlencode_feedback(AT_FEEDBACK_CANCELLED); ?>"><?php echo $_template['no_cancel']; ?></a></center-->
<br />
<?php
	} else if ($_GET['d'] == 1){
		$warnings[]=array(AT_WARNING_SURE_DELETE_MODULE2, $mod_name);
		print_warnings($warnings);
?>
	<?php if($_GET['ad'] == 1){?>
		<?php if($_GET['member_id']){ ?>
			<center><br /><a href="<?php echo $PHP_SELF.'?mod='.$mod.SEP.'d=2'.SEP.'member_id='.$_GET['member_id'].'"'; ?>"><?php echo $_template['yes_delete']; ?></a> | <a href="users/admin/courses.php?f=<?php echo urlencode_feedback(AT_FEEDBACK_CANCELLED).SEP.'member_id='.$_GET['member_id']; ?>"><?php echo $_template['no_cancel']; ?></a></center>
		<?php }else{ ?>
			<center><br /><a href="<?php echo $PHP_SELF.'?mod='.$mod.SEP.'d=2'; ?>"><?php echo $_template['yes_delete']; ?></a> | <a href="users/admin/courses.php?f=<?php echo urlencode_feedback(AT_FEEDBACK_CANCELLED); ?>"><?php echo $_template['no_cancel']; ?></a></center>
		<?php } ?>
		<!--center><br /><a href="<?php echo $PHP_SELF.'?mod='.$mod.SEP.'d=2'; ?>"><?php echo $_template['yes_delete']; ?></a> | <a href="users/admin/courses.php?f=<?php echo urlencode_feedback(AT_FEEDBACK_CANCELLED); ?>"><?php echo $_template['no_cancel']; ?></a></center -->
	<?php }else{ ?>
		<center><br /><a href="<?php echo $PHP_SELF.'?mod='.$mod.SEP.'d=2'; ?>"><?php echo $_template['yes_delete']; ?></a> | <a href="users/?f=<?php echo urlencode_feedback(AT_FEEDBACK_CANCELLED); ?>"><?php echo $_template['no_cancel']; ?></a></center>
	<?php } ?>
	...
	//modif	
	$msql="SELECT * FROM crel_groups WHERE group_id=$mod";
	$mresult=$db->query($msql);
	
	while($mline=$mresult->fetchRow(DB_FETCHMODE_ASSOC)) 
		if ($mline['course_id']!='')
	{
		$course=$mline['course_id'];		
	//modif
		...
		while ($row =$res->fetchRow(DB_FETCHMODE_ASSOC)){
			$sql	= "INSERT INTO del_news VALUES ($row[NEWS_ID], $row[COURSE_ID], $row[MEMBER_ID], '$row[DATA]', $row[FORMATTING], '$row[TITLE]', '$row[BODY]')";			$result = $db->query($sql);
		}
		...
		$res	= $db->query($sql);
		while ($row =$res->fetchRow(DB_FETCHMODE_ASSOC)){
			$sql	= "INSERT INTO del_content VALUES ($row[CONTENT_ID], $row[COURSE_ID], $row[CONTENT_PARENT_ID], $row[ORDERING], TO_DATE('$row[LAST_MODIFIED]', 'DD/MM/YYYY HH24:MI:SS'), $row[REVISION], $row[FORMATTING], TO_DATE('$row[RELEASE_DATE]', 'DD/MI/YYYY HH:MI:SS'), '$row[TITLE]', '$row[TEXT]')";
			$result = $db->query($sql);
		}
		/*$sql	= "SELECT * FROM content WHERE course_id=$course";
		$result = $db->query($sql);
		...
		$sql = "DELETE FROM course_groups WHERE group_id=$mod";
		$res = $db->query($sql);
		$sql = "DELETE FROM crel_groups WHERE group_id=$mod";
		$res = $db->query($sql);
		...
		$feedback[]=AT_FEEDBACK_MODULE_DELETED;
