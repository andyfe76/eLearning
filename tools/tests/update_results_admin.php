<?php
	...
	$sql = "TRUNCATE TABLE bk_course_enrollment";
	$res = $db->query($sql);
	if (PEAR::isError($res)) {
		echo 'Error:';
		print_r($res);
	...
	$sql = "TRUNCATE TABLE bk_mcourse_completion";
	$res = $db->query($sql);
	$sql = "TRUNCATE TABLE bk_tests_results";
	$res = $db->query($sql);
	
	$sql = "INSERT INTO bk_course_enrollment SELECT * FROM course_enrollment";
	$res = $db->query($sql);
	if (PEAR::isError($res)) {
		echo 'Error:';
		print_r($res);
	...
	$sql = "INSERT INTO bk_mcourse_completion SELECT * FROM mcourse_completion";
	$res = $db->query($sql);
	$sql = "INSERT INTO bk_tests_results SELECT * FROM tests_results";
	$res = $db->query($sql);
	...
	while ($row_parse = $res_parse->fetchRow(DB_FETCHMODE_ASSOC)) {
			...
			$consider = false;
			$sql = "SELECT COUNT(*) FROM bk_course_enrollment WHERE member_id=$user_id";
			$res = $db->query($sql);
			$row = $res->fetchRow();
			if ($row[0] >0) $consider = true;
			$sql = "SELECT COUNT(*) FROM bk_mcourse_completion WHERE member_id=$user_id";
			$res = $db->query($sql);
			$row = $res->fetchRow();
			if ($row[0] >0) $consider = true;
		...
		if ($final_score >= 70 )	{
				...
				if ($row[0] >0) {
					...
					$sql  = "UPDATE bk_mcourse_completion SET completed='yes', grade=$final_score, max_grade=$min_grade WHERE member_id=$user_id AND course_id=$_SESSION[course_id] AND completion_date>=TO_DATE('$row[0]', 'DD/MM/YYYY')";
					$res = $db->query($sql);
					if (PEAR::isError($res)) {
					...
					$sql = "INSERT INTO bk_mcourse_completion VALUES ($user_id, $_SESSION[course_id], 'yes', $final_score, $min_grade, SYSDATE)"; 
					$res = $db->query($sql);
					if (PEAR::isError($res)) {
						$subject = "Error tests/view_results";
						$message = $sql;
				...
				$sql = "DELETE FROM bk_course_enrollment WHERE course_id=$_SESSION[course_id] AND member_id=$user_id";
				$res = $db->query($sql);
				//echo "<br>".$sql;
			...
			if ($row[0]<2) {//send only if ther are no retries left 
				...
				$message = "In urma unor imbunatatiri ale platformei eLearning, va anuntam ca rezultatele testelor au fost reverificate. Daca ati mai primit odata aceasta informatie, va rugam ignorati acest email.
