...
*/
	...
	define('CACHE_ON', 0); 		/* Default: 1 - Turn caching on/off */
	define('CACHE_DIR', '/tmp/' . DB_NAME);	/* Default: /tmp/phpCache/ - Default cache directory */
	...
	define('CACHE_STORAGE_CHECKFILE',	CACHE_DIR 
										. '/.phpCache-storage-V'
										. CACHE_VERSION
										. '-HASH='
	...
	function endcache($store=TRUE, $send_output = TRUE) {
		...
		*/
		...
		cache_debug('Caching is done!<br>');
	}
	...
	function cache_gc($dir=NULL, $start=1, $purgeall=FALSE) {
		...
		if (cache_iftype($dir, NULL)) $dir=CACHE_DIR;
		$dp=opendir($dir);
		if (!$dp) {
